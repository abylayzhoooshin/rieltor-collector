...
# Окно на 3 страницы при прогоне раз в 5 минут держит объявление в поле
# зрения дольше, чем нужно full scan'у (раз в 2 часа), чтобы гарантированно
# его захватить, даже если несколько прогонов подряд что-то пропустят.
FAST_LIST_MAX_PAGES = 3
